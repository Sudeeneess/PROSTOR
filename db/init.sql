...
RETURNS TRIGGER AS $$
BEGIN
    -- ЛОГИКА 1: Товар добавлен в заказ (is_ordered: false -> true)
    ...
    -- ЛОГИКА 2: Продажа подтверждена (is_finalized: false -> true)
    IF (TG_OP = 'UPDATE' AND OLD.is_finalized = false AND NEW.is_finalized = true) THEN
        UPDATE warehouse_stock 
    ...
    -- ЛОГИКА 3: Отмена заказа/возврат (если нужно вернуть товар в доступные)
    -- Реализуется по необходимости через проверку статуса возврата
